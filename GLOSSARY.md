# Snipp translation glossary

How the product's own nouns are already rendered in each shipped locale. Generated from the live locale files, so every value below is what users see today.

**Use it as binding.** A new string that contains one of these terms uses the rendering here, not a fresh synonym. If a term genuinely needs to change, change it in every locale file and regenerate this table in the same pass; never let a new key introduce a second spelling.

Terms with no row here (product names like Snipp, Snipp+, Ultra, and file formats) stay in English in every locale.

Source of truth is `Website/utils/i18n/strings/<locale>.json`; `i18n/strings/` mirrors it. Regenerate rather than hand-edit.


## Upload (verb/noun)

`COMMON_UPLOAD` = "Upload"

| Locale | Value |
| --- | --- |
| bg-BG | Качване |
| cs-CZ | Nahrát |
| de-DE | Hochladen |
| el-GR | Ανέβασμα |
| es-ES | Subir |
| fi-FI | Lataa |
| fil-PH | I-upload |
| fr-FR | Envoyer |
| hi-IN | अपलोड |
| hr-HR | Prenesi |
| id-ID | Unggah |
| it-IT | Carica |
| ja-JP | アップロード |
| ko-KR | 업로드 |
| nb-NO | Last opp |
| nl-NL | Uploaden |
| pl-PL | Prześlij |
| pt-BR | Enviar |
| pt-PT | Carregar |
| ro-RO | Încarcă |
| ru-RU | Загрузить |
| sk-SK | Nahrať |
| sr-RS | Otpremi |
| sv-SE | Ladda upp |
| th-TH | อัปโหลด |
| tr-TR | Yükle |
| uk-UA | Завантажити |
| vi-VN | Tải lên |
| zh-CN | 上传 |

## Gallery

`COMMON_GALLERY` = "Gallery"

| Locale | Value |
| --- | --- |
| bg-BG | Галерия |
| cs-CZ | Galerie |
| de-DE | Galerie |
| el-GR | Γκαλερί |
| es-ES | Galería |
| fi-FI | Galleria |
| fil-PH | Galerya |
| fr-FR | Galerie |
| hi-IN | गैलरी |
| hr-HR | Galerija |
| id-ID | Galeri |
| it-IT | Galleria |
| ja-JP | ギャラリー |
| ko-KR | 갤러리 |
| nb-NO | Galleri |
| nl-NL | Galerij |
| pl-PL | Galeria |
| pt-BR | Galeria |
| pt-PT | Galeria |
| ro-RO | Galerie |
| ru-RU | Галерея |
| sk-SK | Galéria |
| sr-RS | Galerija |
| sv-SE | Galleri |
| th-TH | แกลเลอรี |
| tr-TR | Galeri |
| uk-UA | Галерея |
| vi-VN | Thư viện |
| zh-CN | 画廊 |

## Discover

`COMMON_DISCOVER` = "Discover"

| Locale | Value |
| --- | --- |
| bg-BG | Откриване |
| cs-CZ | Objevit |
| de-DE | Entdecken |
| el-GR | Εξερεύνηση |
| es-ES | Descubrir |
| fi-FI | Tutustu |
| fil-PH | Tuklasin |
| fr-FR | Découvrir |
| hi-IN | खोजें |
| hr-HR | Otkrij |
| id-ID | Jelajahi |
| it-IT | Esplora |
| ja-JP | 見つける |
| ko-KR | 둘러보기 |
| nb-NO | Utforsk |
| nl-NL | Ontdekken |
| pl-PL | Odkrywaj |
| pt-BR | Descobrir |
| pt-PT | Descobrir |
| ro-RO | Descoperă |
| ru-RU | Обзор |
| sk-SK | Objavuj |
| sr-RS | Otkrij |
| sv-SE | Utforska |
| th-TH | สำรวจ |
| tr-TR | Keşfet |
| uk-UA | Огляд |
| vi-VN | Khám phá |
| zh-CN | 发现 |

## Share

`COMMON_SHARE` = "Share"

| Locale | Value |
| --- | --- |
| bg-BG | Споделяне |
| cs-CZ | Sdílet |
| de-DE | Teilen |
| el-GR | Κοινοποίηση |
| es-ES | Compartir |
| fi-FI | Jaa |
| fil-PH | Ibahagi |
| fr-FR | Partager |
| hi-IN | शेयर करें |
| hr-HR | Podijeli |
| id-ID | Bagikan |
| it-IT | Condividi |
| ja-JP | 共有 |
| ko-KR | 공유 |
| nb-NO | Del |
| nl-NL | Delen |
| pl-PL | Udostępnij |
| pt-BR | Compartilhar |
| pt-PT | Partilhar |
| ro-RO | Distribuie |
| ru-RU | Поделиться |
| sk-SK | Zdieľať |
| sr-RS | Podeli |
| sv-SE | Dela |
| th-TH | แชร์ |
| tr-TR | Paylaş |
| uk-UA | Поділитися |
| vi-VN | Chia sẻ |
| zh-CN | 分享 |

## Follow

`FOLLOW_FOLLOW` = "Follow"

| Locale | Value |
| --- | --- |
| bg-BG | Последване |
| cs-CZ | Sledovat |
| de-DE | Folgen |
| el-GR | Ακολούθηση |
| es-ES | Seguir |
| fi-FI | Seuraa |
| fil-PH | I-follow |
| fr-FR | Suivre |
| hi-IN | फ़ॉलो करें |
| hr-HR | Prati |
| id-ID | Ikuti |
| it-IT | Segui |
| ja-JP | フォロー |
| ko-KR | 팔로우 |
| nb-NO | Følg |
| nl-NL | Volgen |
| pl-PL | Obserwuj |
| pt-BR | Seguir |
| pt-PT | Seguir |
| ro-RO | Urmărește |
| ru-RU | Подписаться |
| sk-SK | Sledovať |
| sr-RS | Zaprati |
| sv-SE | Följ |
| th-TH | ติดตาม |
| tr-TR | Takip et |
| uk-UA | Стежити |
| vi-VN | Theo dõi |
| zh-CN | 跟随 |

## Views

`COMMON_VIEW_PLURAL` = "views"

| Locale | Value |
| --- | --- |
| bg-BG | прегледа |
| cs-CZ | zhlédnutí |
| de-DE | Aufrufe |
| el-GR | προβολές |
| es-ES | vistas |
| fi-FI | katselua |
| fil-PH | mga tingin |
| fr-FR | vues |
| hi-IN | व्यूज़ |
| hr-HR | pregleda |
| id-ID | tampilan |
| it-IT | visualizzazioni |
| ja-JP | 回表示 |
| ko-KR | 조회 |
| nb-NO | visninger |
| nl-NL | weergaven |
| pl-PL | wyświetleń |
| pt-BR | visualizações |
| pt-PT | visualizações |
| ro-RO | vizualizări |
| ru-RU | просмотров |
| sk-SK | zobrazení |
| sr-RS | pregleda |
| sv-SE | visningar |
| th-TH | ครั้งที่ดู |
| tr-TR | görüntüleme |
| uk-UA | переглядів |
| vi-VN | lượt xem |
| zh-CN | 翻阅 |

## Public

`COMMON_PRIVACY_PUBLIC` = "Public"

| Locale | Value |
| --- | --- |
| bg-BG | Публична |
| cs-CZ | Veřejné |
| de-DE | Öffentlich |
| el-GR | Δημόσιο |
| es-ES | Público |
| fi-FI | Julkinen |
| fil-PH | Pampubliko |
| fr-FR | Public |
| hi-IN | पब्लिक |
| hr-HR | Javno |
| id-ID | Publik |
| it-IT | Pubblico |
| ja-JP | 公開 |
| ko-KR | 공개 |
| nb-NO | Offentlig |
| nl-NL | Openbaar |
| pl-PL | Publiczny |
| pt-BR | Público |
| pt-PT | Público |
| ro-RO | Public |
| ru-RU | Публичный |
| sk-SK | Verejné |
| sr-RS | Javno |
| sv-SE | Offentlig |
| th-TH | สาธารณะ |
| tr-TR | Herkese Açık |
| uk-UA | Публічний |
| vi-VN | Công khai |
| zh-CN | 公开 |

## Private

`COMMON_PRIVACY_PRIVATE` = "Private"

| Locale | Value |
| --- | --- |
| bg-BG | Лична |
| cs-CZ | Soukromé |
| de-DE | Privat |
| el-GR | Ιδιωτικό |
| es-ES | Privado |
| fi-FI | Yksityinen |
| fil-PH | Pribado |
| fr-FR | Privé |
| hi-IN | प्राइवेट |
| hr-HR | Privatno |
| id-ID | Pribadi |
| it-IT | Privato |
| ja-JP | 非公開 |
| ko-KR | 비공개 |
| nb-NO | Privat |
| nl-NL | Privé |
| pl-PL | Prywatny |
| pt-BR | Privado |
| pt-PT | Privado |
| ro-RO | Privat |
| ru-RU | Приватный |
| sk-SK | Súkromné |
| sr-RS | Privatno |
| sv-SE | Privat |
| th-TH | ส่วนตัว |
| tr-TR | Özel |
| uk-UA | Приватний |
| vi-VN | Riêng tư |
| zh-CN | 私有 |

## Unlisted

`COMMON_PRIVACY_UNLISTED` = "Unlisted"

| Locale | Value |
| --- | --- |
| bg-BG | Скрита |
| cs-CZ | Neuvedené |
| de-DE | Nicht gelistet |
| el-GR | Μη καταχωρημένο |
| es-ES | Sin listar |
| fi-FI | Listaamaton |
| fil-PH | Unlisted |
| fr-FR | Non répertorié |
| hi-IN | अनलिस्टेड |
| hr-HR | Neuvršteno |
| id-ID | Tidak Terdaftar |
| it-IT | Non in elenco |
| ja-JP | 限定公開 |
| ko-KR | 링크 공개 |
| nb-NO | Uoppført |
| nl-NL | Verborgen |
| pl-PL | Niepubliczny |
| pt-BR | Não listado |
| pt-PT | Não listado |
| ro-RO | Nelistat |
| ru-RU | По ссылке |
| sk-SK | Neuvedené |
| sr-RS | Nelistirano |
| sv-SE | Olistad |
| th-TH | ไม่อยู่ในรายการ |
| tr-TR | Listelenmemiş |
| uk-UA | Прихований |
| vi-VN | Không công khai |
| zh-CN | 未列出 |

## Unlimited

`COMMON_UNLIMITED` = "Unlimited"

| Locale | Value |
| --- | --- |
| bg-BG | Неограничено |
| cs-CZ | Neomezeně |
| de-DE | Unbegrenzt |
| el-GR | Απεριόριστο |
| es-ES | Ilimitado |
| fi-FI | Rajaton |
| fil-PH | Walang Limitasyon |
| fr-FR | Illimité |
| hi-IN | अनलिमिटेड |
| hr-HR | Neograničeno |
| id-ID | Tak Terbatas |
| it-IT | Illimitato |
| ja-JP | 無制限 |
| ko-KR | 무제한 |
| nb-NO | Ubegrenset |
| nl-NL | Onbeperkt |
| pl-PL | Bez limitu |
| pt-BR | Ilimitado |
| pt-PT | Ilimitado |
| ro-RO | Nelimitat |
| ru-RU | Без ограничений |
| sk-SK | Bez limitu |
| sr-RS | Neograničeno |
| sv-SE | Obegränsat |
| th-TH | ไม่จำกัด |
| tr-TR | Sınırsız |
| uk-UA | Необмежено |
| vi-VN | Không giới hạn |
| zh-CN | 无限 |

## Priority Minutes

`SETTINGS_BILLING_PRIORITY_MINUTES_LABEL` = "Priority Minutes"

| Locale | Value |
| --- | --- |
| bg-BG | Приоритетни минути |
| cs-CZ | Prioritní minuty |
| de-DE | Prioritätsminuten |
| el-GR | Λεπτά προτεραιότητας |
| es-ES | Minutos prioritarios |
| fi-FI | Prioriteettiminuutit |
| fil-PH | Priority Minutes |
| fr-FR | Minutes prioritaires |
| hi-IN | प्राथमिकता मिनट |
| hr-HR | Prioritetne minute |
| id-ID | Menit Prioritas |
| it-IT | Minuti prioritari |
| ja-JP | 優先ストリーミング時間 |
| ko-KR | 우선 스트리밍 시간 |
| nb-NO | Prioritetsminutter |
| nl-NL | Prioriteitsminuten |
| pl-PL | Minuty priorytetowe |
| pt-BR | Minutos prioritários |
| pt-PT | Minutos prioritários |
| ro-RO | Minute prioritare |
| ru-RU | Приоритетные минуты |
| sk-SK | Prioritné minúty |
| sr-RS | Prioritetni minuti |
| sv-SE | Prioritetsminuter |
| th-TH | นาทีลำดับความสำคัญ |
| tr-TR | Öncelikli dakikalar |
| uk-UA | Пріоритетні хвилини |
| vi-VN | Phút ưu tiên |
| zh-CN | 优先分钟数 |

## Weekly Limit

`SETTINGS_BILLING_WEEKLY_LIMIT_LABEL` = "Weekly Limit"

| Locale | Value |
| --- | --- |
| bg-BG | Седмичен лимит |
| cs-CZ | Týdenní limit |
| de-DE | Wöchentliches Limit |
| el-GR | Εβδομαδιαίο Όριο |
| es-ES | Límite Semanal |
| fi-FI | Viikkoraja |
| fil-PH | Lingguhang Limitasyon |
| fr-FR | Limite hebdomadaire |
| hi-IN | साप्ताहिक सीमा |
| hr-HR | Tjedno ograničenje |
| id-ID | Batas Mingguan |
| it-IT | Limite settimanale |
| ja-JP | 週間制限 |
| ko-KR | 주간 한도 |
| nb-NO | Ukentlig grense |
| nl-NL | Wekelijkse limiet |
| pl-PL | Limit Tygodniowy |
| pt-BR | Limite Semanal |
| pt-PT | Limite Semanal |
| ro-RO | Limită săptămânală |
| ru-RU | Недельный лимит |
| sk-SK | Týždenný limit |
| sr-RS | Nedeljni limit |
| sv-SE | Veckogräns |
| th-TH | ขีดจำกัดรายสัปดาห์ |
| tr-TR | Haftalık Sınır |
| uk-UA | Тижневий ліміт |
| vi-VN | Giới hạn hàng tuần |
| zh-CN | 每周限制 |


## Subscription Credit

`SETTINGS_BILLING_CREDIT_HEADING` = "Subscription Credit"

| Locale | Value |
| --- | --- |
| bg-BG | Абонаментен кредит |
| cs-CZ | Kredit na předplatné |
| de-DE | Abo-Guthaben |
| el-GR | Πίστωση συνδρομής |
| es-ES | Crédito de suscripción |
| fi-FI | Tilaushyvitys |
| fil-PH | Kredito sa subscription |
| fr-FR | Crédit d'abonnement |
| hi-IN | सब्सक्रिप्शन क्रेडिट |
| hr-HR | Pretplatnički kredit |
| id-ID | Kredit langganan |
| it-IT | Credito abbonamento |
| ja-JP | サブスクリプションクレジット |
| ko-KR | 구독 크레딧 |
| nb-NO | Abonnementskreditt |
| nl-NL | Abonnementstegoed |
| pl-PL | Kredyt subskrypcyjny |
| pt-BR | Crédito de assinatura |
| pt-PT | Crédito de subscrição |
| ro-RO | Credit de abonament |
| ru-RU | Резерв подписки |
| sk-SK | Kredit predplatného |
| sr-RS | Kredit pretplate |
| sv-SE | Tillgodohavande |
| th-TH | เครดิตการสมัครสมาชิก |
| tr-TR | Abonelik kredisi |
| uk-UA | Резерв підписки |
| vi-VN | Gói đã lưu |
| zh-CN | 待生效的订阅 |
